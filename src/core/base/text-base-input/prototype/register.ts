 ...
 * ```
 */
export const register = function <FieldValuesTypes>(this: ITextBaseInput) {
    const onChange = (e: Event) => {
        ...
        this.field._notifier?.notify(
            'onChange',
            newEvent(this.field.name, onChange.name, 'onChange', `field.${onChange.name}`)
        )
    ...
    const onBlur = (e: Event) => {
        ...
        this.field._notifier?.notify(
            'onBlur',
            newEvent(this.field.name, onBlur.name, 'onBlur', `field.${onBlur.name}`)
        )
    }
