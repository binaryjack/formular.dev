...
export const setValue = function (
    ...
    if (this.field.type === 'checkbox') {
        if (this.field._clickBased) this.field._clickBased.checked = value as boolean
        this.field.dmSetChecked(this.field.id.toString(), value as boolean)
        this.field.value = value
    } else if (this.field.type === 'radio') {
        const radioItem = this.field._optionBased?.tryGetOptionByIdOrValue(
            value as string,
            value as string
        )
        if (!radioItem) {
        ...
        if (this.field._optionBased) this.field._optionBased.selectedOptionId = radioItem.sequenceId
        this.field.dmSetChecked(radioItem.id, value as boolean)
    } else if (this.field.type === 'select') {
        const optionById = this.field._optionBased?.tryGetOptionBySequenceIdThenIdOrValue(
            value as number,
            value as string,
            value as string
        )
        ...
        if (this.field._optionBased)
            this.field._optionBased.selectedOptionId = optionById.sequenceId
