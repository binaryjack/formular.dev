...
export function getValue(this: IValueStrategy, field: IFieldInput): unknown | null {
    const strategy = this.strategies.find((s) => s.concernedTypes.includes(field.type))
    ...
    try {
        ...
        switch (strategy.fieldValueProperty) {
            ...
            case 'selectedOptionId':
                return strategy.method(field._optionBased?.selectedOptionId as FieldValuesTypes)
            case 'value':
            default:
