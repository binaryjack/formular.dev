...
export const observe = function <T extends object>(
    ...
    const introspectionEnabled = this._introspectionEnabled
    ...
    const debugStreamMaxSize = this._debugStreamMaxSize || 100
    const delay =
        options?.debounceDelay ?? this.observablesDelay ?? this.inputDelay ?? 100

    // Create wrapper callback that logs to debugStream
    const wrappedCallback = () => {
        // Log to debug stream (ring buffer)
        const debugStream = this.debugStream || []
        debugStream.push({
            type: 'observer_fired',
